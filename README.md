...
The workflow also commits updated `.cache/` variants back to `main` so future
builds can reuse them immediately.
...
4. **About page** — `content/about.md` is rendered to HTML at build time and injected directly into `_site/about.html`. `content/profile.jpg` (if present) is optimized to responsive WebP variants (400w, 800w, 1200w, 1920w at quality 85), cached in `.cache/`, and a `<link rel="preload" as="image" fetchpriority="high">` hint is added to `<head>` with responsive srcset hints. The `<img>` tag in about.html is updated with `srcset` for responsive image loading. The raw `.md` file is never included in `_site/`.
5. **Image optimisation** — every source image under `content/photos/` is processed with `sharp` into four WebP variants (400w, 800w, 1200w, 1920w at quality 85) and written to `_site/photos/`. For the cover image of each collection, an additional `{stem}-og.webp` (1200×630 centre-crop, quality 80) is generated for use in OG/Twitter meta tags — sized to stay under WhatsApp's 600 KB limit. Results are cached in `.cache/` (keyed by SHA-256 hash of the source file) so unchanged images are skipped on subsequent builds. The build prunes stale cache entries/files when source images or folders are removed/renamed, so only changed files are regenerated.
...
The `_site/` folder is listed in `.gitignore` and is never committed — it is regenerated from scratch on every build. The `.cache/` folder is committed and maintained by CI; workflow runs update it only when source image content changes.
